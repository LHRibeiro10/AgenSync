    ...
    <Card className="min-w-0 overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-[#E2E8F0] p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 truncate text-xl font-black tracking-tight text-ink">{client.name}</h2>
          <p className="mt-1 text-sm leading-6 text-muted">{client.phone}</p>
        </div>
      ...
      <div className="min-w-0 p-3 sm:p-5 lg:p-6">
        <div key={renderedTab} className={leavingTab ? "tab-panel-exit" : "tab-panel-enter"}>
          {renderedTab === "data" ? (
            <ClientDataTab client={client} care={care} appointments={appointments} onEdit={onEdit} />
          ) : null}
