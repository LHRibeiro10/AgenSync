    ...
    <div className="space-y-5">
      ...
      {summary ? (
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <StatTile label="Usuarios" value={summary.totalUsers || 0} detail={`${summary.newUsers7Days || 0} novos em 7 dias`} />
            <StatTile label="Admins" value={summary.admins || 0} detail="Usuarios com acesso administrativo" />
            <StatTile label="Logins 7 dias" value={summary.logins7Days || 0} detail={`${summary.loginFailures7Days || 0} falhas recentes`} />
            <StatTile label="Acessos admin" value={summary.adminAccess7Days || 0} detail="Ultimos 7 dias" />
          </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        <Card className="overflow-hidden">
          <CardHeader title="Usuarios" description="Lista administrativa com role interno de acesso." />
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((user) => (
                  <tr key={user.id} className="bg-white align-middle">
                    <td className="px-4 py-3 font-black text-ink">{user.name}</td>
      ...
      <Card className="overflow-hidden">
        <CardHeader title="Auditoria de acesso e uso" description="Ultimos eventos administrativos e de autenticacao." />
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            <thead className="bg-slate-50 text-xs font-black uppercase tracking-[0.14em] text-muted">
              <tr>
